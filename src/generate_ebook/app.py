...
def lambda_handler(event, context):
    ...
    front_cover_url = (
        payload.get("front_cover_image_url")
        or payload.get("hardcover_front_image_url")
    )
    if front_cover_url:
        cover_data = download_image(front_cover_url)
        if cover_data:
            book.set_cover("cover.jpg", cover_data)
            print("EPUB cover set from front cover image.")
        else:
            print("Could not download front cover image for EPUB cover.")

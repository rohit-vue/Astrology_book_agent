...
def build_chapter_batch_tasks(chapters_list, astrology_data, focus, style, language, word_target):
    ...
    for idx, ch in enumerate(chapters_list):
        ...
        prompt = (
            ...
            f"Special multi-line blocks (mandatory when the content needs them â€” do not suppress):\n"
            ...
            f"- A paragraph containing inline enumeration like \"1. ... 2. ... 3. ...\" "
            f"should be rewritten as a vertical list, unless the numbers are dates, quantities, "
            f"astrological placements, or other factual values inside normal prose.\n"
            f"- Use bullet lists for non-sequential grouped ideas; use numbered lists for ordered "
            f"checks, steps, questions, criteria, or exercises.\n"
...
def collect_chapter_batch_results(client, batch, manifest, artifact_prefix="chapter_text"):
    ...
    for line_num, line in enumerate(result_content.decode("utf-8").strip().split("\n"), 1):
        ...
        cleaned_text = cleanup_inline_lists(chapter_text)
        if cleaned_text != chapter_text:
            print("    Reflowed inline list markers into vertical list blocks")
